import { LayoutGrid, GitBranch, ListOrdered, Share2 } from "lucide-react";
...
export default function MobileNav({ view, setView }) {
  return (
    <div className="md:hidden fixed bottom-3 left-3 right-3 z-30">
      <div
        className="flex items-center justify-between rounded-2xl px-2 py-2 shadow-xl"
        ...
        {ITEMS.map((it) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9.5px] font-medium" style={{ color: active ? "var(--color-paper)" : "var(--color-stone)" }}>
                {it.label}
              </span>
            </button>
          );
